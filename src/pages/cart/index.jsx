...
const Cart = () => {
    ...
    const deleteItem = async (id) => {
        const response = await axios.delete(`http://localhost:5173/cartItems/${id}`);
        if (response !== null) {
            context.getCartData("http://localhost:5173/cartItems");
    ...
    const emptyCart = () => {
        ...
        cartItems.length !== 0 &&
            cartItems.map((item) => {
                response = axios.delete(`http://localhost:5173/cartItems/${parseInt(item.id)}`);
            })
        if (response !== null) {
            context.getCartData("http://localhost:5173/cartItems");
        }
